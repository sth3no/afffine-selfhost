-- Phase 14.2: rewrite the (*, *) seed system_prompt to (a) teach the LLM
-- AFFiNE's specific markdown dialect, and (b) work well as the REDUCE-step
-- prompt for long transcripts processed via the map-reduce chunker.
--
-- Production hit two more issues after 14.1:
--   1. The LLM produced markdown that didn't render correctly in AFFiNE
--      because it didn't know which markdown features AFFiNE supports
--      (HTML tags, GFM tables, deep nesting → all broken). The new prompt
--      embeds an explicit syntax reference.
--   2. With the map-reduce path for long transcripts, the prompt sees
--      chunk-summary digests in addition to (or instead of) the raw body.
--      The new prompt acknowledges both shapes.
--
...
- Embedded HTML/SVG frames: ```embed-html\n<svg ...>...</svg>\n``` →
  ...
  visualizations.
- Inline links: `[label](url)` → clickable. Always use this for URLs in
  prose; never paste bare URLs.
...
- Cross-doc references: `[[Doc Title]]` → embed-linked-doc block when the
  ...
  related-notes links.
- Callouts: a line `> [!callout] text` → highlighted callout block. Use
  for warnings, key insights, TL;DR. Single-line only.
- Image references to extracted keyframes: `![caption](kf:<index>)` where
  `<index>` is the 0-based index from the "Available keyframes" list the
  user message will provide. Use sparingly — only when a frame
  substantively supports the surrounding text.
...
- The user message may include a "Source description" block (publisher's
  ...
  links, chapter markers, related-content links. Surface valuable
  references in `body_md` as a `## Sources` section. Strip
  sponsor / affiliate / social-media noise.
